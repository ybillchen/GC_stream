...
int main(int argc, char const *argv[])
{
    ...
    double p_between = potV->value(coord::PosCar(0.,0.,0.), r) * energyUnit;

    std::cout.precision(4);
    std::cout << "center potential at t = 0: " << p0 << " km2/s2" << std::endl;
    std::cout << "center potential at t = " << r << ": " << p_between << " km2/s2" << std::endl;
    std::cout << "center potential at t = 1: " << p1 << " km2/s2" << std::endl;

    bool passed = (fabs((1-r)*p0 + r*p1 - p_between) / fabs(p_between) < 1e-6);
